...
class Controller extends BaseController
{
    ...
    function add_feature($data){        
            $id = \App\Features::where('data', $data->get('feature'))->pluck('id')->all();            
            if(!$id){
    ...
    function features_rel (Request $request){
        $data=DB::table('features_parents')->where('feature_id',$request->get('id'))
                                           ->join('areas','features_parents.parent_id','=','areas.id')                                           
                                           ->get();
                ...
                if($object_data[0]->$addon){
                    if($addon=='features'){
                       ...
                       $addon_data=DB::table('features_parents')->where('parent_id',$request->get('id'))
                                                                   ->join('features', 'features_parents.feature_id','=','features.id')
                                                                   ->join('feature_group', 'features.feature_group', '=', 'feature_group.id')
                                                                   ->get();
